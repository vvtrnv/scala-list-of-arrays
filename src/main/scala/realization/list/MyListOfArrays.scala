...
class MyListOfArrays {
  ...
  private def mergeSortFunc(arr: Array[AnyRef], comporator: Comporator): Array[AnyRef] = {
    if (arr.length <= 1) {
      arr
    } else {
  ...
  private def mergeFunc(left: Array[AnyRef], right: Array[AnyRef], comporator: Comporator): Array[AnyRef] = {
    (left, right) match {
  ...
  private def arraysInOneArray(): Array[AnyRef] = {
    // Переносим все элементы в один массив
